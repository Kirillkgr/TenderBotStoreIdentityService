...
import kirillzhdanov.identityservice.service.UserDetailsServiceImpl;
...
public class DatabaseInitializer {
    ...
    public void init(){
        ...
        for(int i = 1; i<=3; i++) {
            ...
            Brand brand = brandRepository.existsByName(brandName) ? brandRepository.findAll().stream().filter(b->b.getName().equals(brandName)).findFirst().orElse(null) : brandRepository.save(Brand.builder().name(brandName).telegramBotToken(token).build());
            brands.add(brand);
        }
        log.info("Brands: {}", brands.stream().map(Brand::getName).toList());
        ...
        for(int i = 1; i<=5; i++) {
            ...
            User user = userRepository.saveNewUser(User.builder().username(username).password("password" + i).build());
            user.setBrands(userBrands);
            user.setRoles(roles);
            userRepository.save(user);
            log.info("Created user: {} with roles: {} and brands: {}", username, roles.stream().map(r->r.getName().name()).toList(), userBrands.stream().map(Brand::getName).toList());
        }
    }
