...
public class BotService {
	...
	public MessageResponse registerBot(BotRegistrationRequest request){
		...
		Brand brand = brandRepository.findById(request.getBrandId()).orElseThrow(()->new ResourceNotFoundException("Бренд не найден"));

		// Сохраняем токен в бренд
		brand.setTelegramBotToken(request.getBotToken());
		brandRepository.save(brand);

		return MessageResponse.builder().message("Бот успешно зарегистрирован для бренда: " + brand.getName()).build();
	}

	public MessageResponse startBot(BotRegistrationRequest request){
		// Проверяем, что бренд с таким токеном существует
		Brand brand = brandRepository.findAll().stream().filter(b->request.getBotToken().equals(b.getTelegramBotToken())).findFirst().orElseThrow(()->new ResourceNotFoundException("Бот с таким токеном не найден"));
		//TODO: Start botService
		// Здесь будет логика запуска бота
		return MessageResponse.builder().message("Бот запущен для бренда: " + brand.getName()).build();
	}
}
