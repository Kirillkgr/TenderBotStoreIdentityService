  ...
  <div class="modal-backdrop">
    <div class="modal">
      ...
      <footer class="modal-footer">
        ...
        <button :disabled="!canSubmit || loading" class="btn danger" @click="onSubmit">
          <span v-if="loading">Списание…</span>
          <span v-else>Списать</span>
        </button>
      </footer>
    </div>
  </div>
...
async function onSubmit() {
  ...
  try {
    ...
    error.value = e?.response?.data?.message || e?.message || 'Не удалось выполнить списание';
  } finally {
    loading.value = false;
  }
}
